...
pro save_beam_data, date_s, date_e, output_path, all_tplot_names
  headers =  ['Time' $            
              , 'En', 'Pa_para', 'Flux_para' , 'Eflux_para' $
              , 'Pa_anti', 'Flux_anti' , 'Eflux_anti' $
             ]
  data_tplot_names_x = all_tplot_names.x_gse_name
...
; anti data  
  ...
  data_dd_temp = [REFORM(time_avg,1,n_total), REFORM(en_flat,1,n_total) $
                ,  REFORM(flux_pa_para_flat,1,n_total), REFORM(flux_para_flat,1,n_total),  REFORM(eflux_para_flat,1,n_total) $
                  , REFORM(flux_pa_anti_flat,1,n_total), REFORM(flux_anti_flat,1,n_total),  REFORM(eflux_anti_flat,1,n_total) $
                ]
  
  index = WHERE(TOTAL(data_dd_temp[2:(N_ELEMENTS(data_dd_temp(*,0))-1),*],1,/NAN) NE 0, ct)
  IF ct GT 0 THEN data_dd = data_dd_temp[*,index] 
