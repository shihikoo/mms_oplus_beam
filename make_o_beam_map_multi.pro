...
PRO make_o_beam_map_multi, odata, header $
...
;-- default keywords settings if keywords are not set --
  IF NOT KEYWORD_SET(grid) THEN grid = 3. & grid_str = STRING(grid, format = '(i2.2)')
...
;  IF NOT KEYWORD_SET(sample_size_threshold) THEN  sample_size_threshold = 27
  ...
  IF NOT KEYWORD_SET(sort_flag) THEN sort_flag = {sort_flag: 1, sort_flag_fulldata:1}
