...
; open the ps plot
  ...
  PLOT, [0, 0, -100, 100], [-100, 100, 0, 0] $
        ...
        , XSTYLE = 5, ystyle = 5, charsize = 1.5, position = [0.15, 0.15, 0.85, 0.85], xlog = xlog, ylog = ylog
  
  IF keyword_set(ps_plot) then psym_point_map = 1 else psym_point_map = 3

; plot data that has no beam
  oplot, data_pos_ilat*(flag_para EQ 0 AND flag_anti EQ 0), data_pos_mlt*(flag_para EQ 0 AND flag_anti EQ 0), color = 5, psym = psym_point_map,/polar
...
; legend         
  ...
  xyouts, la_x+7, la_y, 'beam events', color = 2
...
; open the ps plot
  ...
  PLOT, [0, 0, -100, 100], [-100, 100, 0, 0] $
        ...
        , charsize = 1.5, position = [0.15, 0.15, 0.85, 0.85], xlog = xlog, ylog = ylog

  if keyword_set(ps_plot) then psym_point_map=1 else psym_point_map=3

; plot data without beams      
  oplot, data_pos(*, 0)*(flag_para EQ 0 AND flag_anti EQ 0), data_pos(*, 1)*(flag_para EQ 0 AND flag_anti EQ 0), symsize=0.5, psym = psym_point_map, color = 5
; plot data with beams
...
; legend         
  ...
  xyouts, la_x, la_y, 'beam evnets', color = 2
...
PRO make_points_map, data_pos, flag_para, flag_anti, beta, mlt, range, log, path_ps, ts_date, te_date, plot_axis, grid, sort_title, phase, region, direction, ps_plot = ps_plot
  ...
  filename =  path_ps + 'footage/'+direction+'_'+phase +'_'+ region + '_sampling_' + ts_date+'_to_'+ te_date+'_' + PLOT_AXIS(0)+'_vs_'+PLOT_AXIS(1) +'.ps'
  title =  sort_title+' '+direction+' '+phase + ' O!U+!N SAMPLES in '+ region +'!Cfrom  ' +ts_date+'  to  ' +te_date
