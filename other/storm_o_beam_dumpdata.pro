PRO storm_o_beam_dumpdata

  sc = 4
  sc_str = STRING(sc, FORMAT = '(i1.1)')
  ps = 0 ; program will plot ps files only when ps EQ 1
  dumpdata = 1; program will dump data out only when dumpdata EQ 1 
...
;---------------------------------------------------------------------
  ...
  OPENU, unit, 'log_plotted_dump.txt', /GET_LUN, /APPEND
  PRINTF, unit, SYSTIME(), '[START]'
  FREE_LUN, unit         
 
  OPENU, unit, 'log_errors_dump.txt', /GET_LUN, /APPEND
  PRINTF, unit, SYSTIME(), '[START]'
  FREE_LUN, unit         
          ...
          ;       options, p08, 'ytitle', 'SC' + sc_str + '!C!CBx (nT)'
                  ...
                  FOR i_en = 0, n_en_names-1 DO BEGIN 
                      ...
                      options, p11+en_names(i_en)+'_pap_beam', 'ytitle', $
                               'O!U+!N  beam(T'+en_names(i_en)+')' + '!C!C' + 'E-------T'
                      options, p12+en_names(i_en)+'_pap_beam', 'ytitle', $
                               'O!U+!N  beam(E'+en_names(i_en)+')' + '!C!C' + 'E-------T'
              ...
              ;---------------------------------------
                  ...
                  date_ts = STRMID(ts, 0, 4) + STRMID(ts, 5, 2) + STRMID(ts, 8, 2)
                  time_ts = STRMID(ts, 11, 2) + STRMID(ts, 14, 2) + STRMID(ts, 17, 2)
                  
                  IF ps EQ 1 THEN BEGIN  
                     
                      fln = 'plots/tem_c/storm_o_beam_' + date_ts + '_' + time_ts + '.ps'
               ...
               ;--------------------------------------
                 ...
                 IF dumpdata EQ 1 THEN BEGIN 
                     ...
                     ds = time_string(time_dd(0))  
                     de = time_string(time_dd(n_time-1))
                     date_ds = STRMID(ds, 0, 4) + STRMID(ds, 5, 2) + STRMID(ds, 8, 2)
                     time_ds = STRMID(ds, 11, 2) + STRMID(ds, 14, 2) + STRMID(ds, 17, 2)
                     date_de = STRMID(de, 0, 4) + STRMID(de, 5, 2) + STRMID(de, 8, 2)
                     time_de = STRMID(de, 11, 2) + STRMID(de, 14, 2) + STRMID(de, 17, 2)
                     ...
                     IF date_ds EQ date_de THEN BEGIN 
                         str = {x:time_dd, y:data_dd, v:title_dd}
                         store_data, 'dump_data', data = str
                         dump_data, 'dump_data', $
                                    file_out = 'data/storm_o_beam_'+date_ds+'.dat'

                     ENDIF ELSE BEGIN 
                         
                         midnight = time_double(STRMID(de, 0, 10)+'/00:00:00')
                         sday = where(time_dd LT midnight)
                         eday = where(time_dd GE midnight)
                         ...
                         str = {x:time_dd(eday), y:data_dd(eday, *), v:title_dd(eday, *)}
                         store_data, 'dump_data_e', data = str
                         dump_data, 'dump_data_e', $
                                    file_out = 'data/storm_o_beam_'+date_de+'.dat'
                 ENDELSE 
;stop
             ENDIF 
;stop
