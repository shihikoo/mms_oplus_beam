PRO slice2d_sim_tem,cal_sim_tem = cal_sim_tem, plot_sim_tem = plot_sim_tem, nt = nt, ps = ps
...
path = 'output/o_beam/test_mom/'+strcompress(nt, /remove_all)+'/'
spawn,  'mkdir '+path
...
;----------------------------------------------------------------------
        ...
        FOR in = 0, 1 DO BEGIN 
            ...
            spawn,  'mkdir '+plot_path
            IF keyword_set(plot_globe) THEN  spawn,  'mkdir '+plot_path+'globe/'
            ...
            v_sim = fltarr(nt*ntt, 3, n_elements(energy_set))
            v_in = fltarr(nt*ntt, 3, n_elements(energy_set))
...
;default enregy set to 440 eV
                ...
                FOR itt = 0, ntt-1 DO BEGIN 
                    FOR it = 0, nt-1 DO BEGIN 
                        IF keyword_set(raw_counts) THEN t_input = 15 ELSE t_input = 40000d^(1.*(it+30.)/nt)
; t_input = (it+1)*(energy_input/nt)*(0.25^itt)
                        IF t_input GT energy_input AND keyword_set(beam_like_dis) THEN BEGIN
                            t_sim(itt*nt+it, *, ien_sort) = !VALUES.F_NAN
                            t_in(itt*nt+it, *, ien_sort) = !VALUES.F_NAN
                        ENDIF ELSE BEGIN 
...
; Velocity In GSE co-ordinates
                                ...
                                IF density_sim LT n0 THEN BEGIN
                                    ...
                                    if loop_times lt 1e2 or f ne 0 then GOTO, next else invalid=1
                                ENDIF
                            ENDIF

;Jing: add temperature calculation part 
                            if not keyword_set(invalid) eq 1 then begin 
                            angle = [[-90.0, 90.0], [0.0, 360.0]] ; bin range to sum over
                            energy = [30.0, 40000.0]
...
; Jing add the velocity calculation part
                    ...
                    ;        stop
                            v_sim(itt*nt+it,*,ien_sort) = vv
                            v_in(itt*nt+it,*,ien_sort) = [v01,v02,v03]
                        endif else begin 
                            t_sim(itt*nt+it, *, ien_sort) = !values.f_nan
                            t_in(itt*nt+it, *, ien_sort) = !values.f_nan
                            v_sim(itt*nt+it, *, ien_sort) = !values.f_nan
                            v_in(itt*nt+it, *, ien_sort) = !values.f_nan
                        endelse 
                        ENDELSE  
                    ENDFOR    
                ENDFOR        
                ...
                v_sim(*, 2, ien_sort) = v_sim(index, 2, ien_sort)
                v_in(*, 0, ien_sort) = v_in(index, 0, ien_sort)
                v_in(*, 1, ien_sort) = v_in(index, 1, ien_sort)
                v_in(*, 2, ien_sort) = v_in(index, 2, ien_sort)
                v_tot_in(*, ien_sort) = sqrt(v_in(*, 0, ien_sort)^2+v_in(*, 1, ien_sort)^2+v_in(*, 2, ien_sort)^2)
                v_tot_sim(*, ien_sort) = sqrt(v_sim(*, 0, ien_sort)^2+v_sim(*, 1, ien_sort)^2+v_sim(*, 2, ien_sort)^2)
            ENDFOR        
            !p.multi = [0, 1, 2]
;draw for dt
            ...
            ENDIF 
;for T
            if keyword_set(plot_linear) then begin 
            FOR j_en_sort = 0, n_en_sort-1 DO BEGIN 
                ...
                FOR it = 0, 2 DO BEGIN 
                    IF keyword_set(ps) THEN $
                      popen, plot_path+'T_test_t'+string(it, format = '(i1)') $
                      + energy_input_str+'.ps', /port $
                    ELSE window, it+3
...
;for T in log
            ...
            store_data, year+'_'+n_condi+'_V_tot', data = {x:v_tot_in, y:v_tot_sim/v_tot_in}
            store_data, year+'_'+n_condi+'_V_para', data = {x:v_in(*, 0, *), y:v_sim(*, 0, *)/v_in(*, 0, *)}
            store_data, year+'_'+n_condi+'_V_perp1', data = {x:v_in(*, 1, *), y:v_sim(*, 1, *)/v_in(*, 1, *)}
            store_data, year+'_'+n_condi+'_V_perp2', data = {x:v_in(*, 2, *), y:v_sim(*, 2, *)/v_in(*, 2, *)}

            tplot_names, '*n_T*', names = names1
            tplot_names,'*n_V*',names=names2
            tplot_save, [names1,names2], filename = fln
        ENDFOR    
    ENDFOR    
ENDIF   

IF keyword_set(plot_sim_v) THEN BEGIN 
    IF v_condi EQ 'sort_en' THEN  n_en_sort = n_elements(energy_set) ELSE  n_en_sort = 1             
    v_tot_in_set = FLTARR(5, 2, nt*ntt, n_en_sort)
    v_ratio_tot_set = FLTARR(5, 2, nt*ntt, n_en_sort)
    v_para_in_set = FLTARR(5, 2, nt*ntt, n_en_sort)
    v_ratio_para_set = FLTARR(5, 2, nt*ntt, n_en_sort)
    v_perp1_in_set = FLTARR(5, 2, nt*ntt, n_en_sort)
    v_ratio_perp1_set = FLTARR(5, 2, nt*ntt, n_en_sort)
    v_perp2_in_set = FLTARR(5, 2, nt*ntt, n_en_sort)
    v_ratio_perp2_set = FLTARR(5, 2, nt*ntt, n_en_sort)
    ...
    FOR iy = 0, 4 DO BEGIN 
        ...
        FOR  in = 0, 1 DO BEGIN
            ...
            get_data, year+'_'+n_condi+'_V_para', data = data
            v_para_in_set(iy, in, *, *) = data.x
            v_ratio_para_set(iy, in, *, *) = data.y
            get_data, year+'_'+n_condi+'_V_perp1', data = data
            v_perp1_in_set(iy, in, *, *) = data.x
            v_ratio_perp1_set(iy, in, *, *) = data.y
            get_data, year+'_'+n_condi+'_V_perp1', data = data
            v_perp2_in_set(iy, in, *, *) = data.x
            v_ratio_perp2_set(iy, in, *, *) = data.y
        ENDFOR 
    ENDFOR 

    IF keyword_set(Verr_plot_3d) THEN BEGIN 
        FOR iy = 0, 4 DO BEGIN 
            FOR in = 0, 1 DO BEGIN 
                year = year_set(iy)
                v_input = reform(v_tot_in_set(iy, in, *, *))
                error = ABS(reform(abs((v_ratio_tot_set(iy, in, *, *)))-1))
                x_axis = v_input(*, 0)
                y_axis = energy_set
                v = error
                
                IF keyword_set(ps) THEN popen, path+Year_set(iy)+'_'+n_condi_set(in)+'_3d_Verror.ps', /land
                specplot, x_axis, y_axis, v, $
                  no_interp = 1, $
    ...
    IF keyword_set(plot_sim_tem) THEN BEGIN 
        ...
        IF keyword_set(Terr_plot_3d) THEN BEGIN 
            FOR iy = 0, 4 DO BEGIN 
                FOR in = 0, 1 DO BEGIN 
                    year = year_set(iy)
                    t_input = reform(ABS(t_tot_in_set(iy, in, *, *)-1))
                    error = ABS(reform((t_ratio_tot_set(iy, in, *, *))))
                    x_axis = t_input(*, 0)
                    ...
                    v = error
                                ...
                                ;       index = sort(x_axis)
                                ;      x_axis = x_axis(index)
                                ;       FOR ien_sort = 0, n_en_sort-1
;       DO v(*, ien_sort) = v(index, ien_sort)
