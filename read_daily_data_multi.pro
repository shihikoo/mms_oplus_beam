...
;     if keyword_set(avoid_compression_time) and start_date_double ge time_double('2019-04-17') and start_date_double le time_double('2019-08-17') then continue
  ...
  if keyword_set(only_no_compress) then begin
     ...
     averaged_filename = 'data/hpca_science_mode_modified_averaged'
     ...
     IF ct_tplot EQ 0 THEN BEGIN     
        ...
        average_tplot_variable_with_given_time, modifed_filename, average_time, data[0,*],/new
        tplot_save, averaged_varname, file =  average_filename
     endif
     
     tplot_restore, filenames = averaged_filename+'.tplot'
     get_data, averaged_varname, data = mode_avg_data
     if datatype eq 'external' then begin
        index = where(mode_avg_data.y gt 4,ct)
     endif else begin
...
;---------------------------------------------------------------------------------------------------------
FUNCTION read_daily_data_multi, time_start, time_end, tplot_path, data_path, read_from_dat = read_from_dat,store_tplot=store_tplot, avoid_compression_time = avoid_compression_time, avoid_2019 = avoid_2019, only_no_compress = only_no_compress
