...
PRO make_o_beam_map_multi_mms, odata, header $
...
;-- Run plotting routine for different conditions
  FOR icondition = 0, N_ELEMENTS(condition_set[*,0])-1 DO BEGIN 
     ...
     IF KEYWORD_SET(EVENTS_MAP) THEN make_events_map_mms, data_pos, data.flag_para, data.flag_anti, path_ev, ts_date, te_date, plot_axis, ext_condition_str, int_condition_str, range, log, grid, slice_grid, filename, plot_2d, plot_slice, make_table, ps_plot = ps_plot, threshold = sample_size_threshold,total_counts = total_counts, region = region, ratio_correction = ratio_correction,energy_range = energy_filter
