...
PRO identify_beams_mms, sat, specie, flux_name, counts_name,  average_time, time_avg  $  
                    , region_name $
                    , t_s = t_s, t_e = t_e $
...
;-------------------------------------------------------------------
  if keyword_set(multi_peak) then begin
     ...
     IF ~KEYWORD_SET(names)  THEN BEGIN
        find_energy_range_from_enspec_multi, flux_avg_name, epcut_name, erange_name, min_erange_bins =1, round_energybins = 1
     ENDIF 
; plot pitch angle in 'DIFF FLUX' 
     tplot_names, pa_name, names = names
     IF ~KEYWORD_SET(names)  THEN plot_pa_spec_around_energy_peak_mms_multi, sat, specie, 'DIFF FLUX' $
        , epcut_name, erange_name, pa_name = pa_name $
        , average_time = average_time

; plot pitch angle for IN 'EFLUX' unit
     tplot_names, pa_eflux_name, names = names    
     IF ~KEYWORD_SET(names) THEN  plot_pa_spec_around_energy_peak_mms_multi, sat, specie, 'EFLUX' $
...
; energy range
     ...
     IF ~KEYWORD_SET(names) THEN $
        filter_beams_multi, pap_name, epcut_name, erange_name, pap_beam_name, epcut_beam_name, erange_beam_name, diff_en = diff_en, diff_pa = diff_pa
...
; plot pitch angle in 'DIFF FLUX' with routine plot_pa_spec_around_energy_peak 
     tplot_names, pa_name, names = names
     IF ~KEYWORD_SET(names)  THEN plot_pa_spec_around_energy_peak_mms, sat, specie, 'DIFF FLUX' $
        , epcut_name, erange_name, pa_name = pa_name $
        , average_time = average_time

; plot pitch angle for IN 'EFLUX' unit
     tplot_names, pa_eflux_name, names = names    
     IF ~KEYWORD_SET(names) THEN  plot_pa_spec_around_energy_peak_mms, sat, specie, 'EFLUX' $
        , epcut_name, erange_name, pa_name = pa_eflux_name $
        , average_time = average_time
