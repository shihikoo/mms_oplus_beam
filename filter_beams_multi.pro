     ...
     FOR j = 0, npa-1 DO BEGIN 
        for k = 0, nen - 1 do begin
           IF flux_beam(i, j,k) GT 0 THEN BEGIN 
              l = TOTAL(flux_beam(i-1, ((j-diff_pa) > 0):((j+diff_pa) < (npa-1)), ((k-diff_e) > 0):((k+diff_e) < (nen-1))) GT 0) GT 0
              r = TOTAL(flux_beam(i+1, ((j-diff_pa) > 0):((j+diff_pa) < (npa-1)), ((k-diff_e) > 0):((k+diff_e) < (nen-1))) GT 0) GT 0
              beam_flag(i, j,k) = l + r
           ENDIF
        ENDFOR
     endfor 
     ...
     FOR j = 0, npa-1 DO BEGIN
        for k = 0, nen - 1 do begin
           IF beam_flag(i, j, k) EQ 2 THEN BEGIN
              temp =  beam_flag(i-1, (j-diff_pa > 0):(j+diff_pa < (npa-1)),  ((k-diff_e) > 0):((k+diff_e) < (nen-1)))
              index = WHERE(temp EQ 1, ct)
              IF ct GT 0 THEN BEGIN 
                 temp[index] = temp[index] + 1
                 beam_flag(i-1, (j-diff_pa > 0):(j+diff_pa < (npa-1)),  ((k-diff_e) > 0):((k+diff_e) < (nen-1))) = temp
              ENDIF
